...
import { TcardProps } from "@repo/middleware/type";
...
export default function Tab({ idTab }: { idTab: TtabProps }): ReactElement {
  ...
  return (
    <section className="py-10">
      <Tabs
        defaultValue={
          idTab.TabDefault.defaultTab ? idTab.TabDefault.defaultTab : "all"
        }
        className="w-full"
      >
        <TabsList
          className={`grid grid-cols-2 md:grid-cols-${Math.min(Lacategories.length + 1, 6)} mb-8 md:gap-0 gap-4 rounded-none bg-accent`}
        >
          <TabsTrigger className="rounded-none" value="all">
            {idTab.TabDefault.text}
          </TabsTrigger>
              ...
              <div className="mt-8 text-center">
                {VisibleCount < idTab.data.length ? (
                  <Button onClick={fnShowMoreItems} size="lg" variant="outline">
                    {idTab.TabDefault.AllLabel}{" "}
                    <ChevronDown className="size-5" />
                  </Button>
                ) : (
                  <Button
                    onClick={() => fnSetVisibleCount(4)}
                    size="lg"
                    variant="outline"
                  >
        ...
        {Lacategories.map((iCategory) => (
          <TabsContent key={iCategory} value={iCategory}>
            <div className="grid sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 mt-4 gap-6">
              {idTab.data
                .filter(
                  (idItem) => (idItem.category ?? "uncategorized") === iCategory
                )
                .map((idItem, iIndex) => (
                  <CustomCard key={iIndex} idCardProps={idItem} />
                ))}
