...
function fnGetClassNameFromFriendlyName(iStrapiValue: string) {
  ...
  const LdClassNameMap: Record<string, string> = {
    ...
    Half_Width_on_Tablet_Right_Padding_Medium_2_5:
      "w-full md:w-1/2 md:pr-2.5 mb-3",
    Half_Width_on_Tablet_Left_Padding_Medium_2_5:
      "w-full md:w-1/2 md:pl-2.5 mb-3",
 ...
 */
export async function fnSubmitWebinar(
  idFormData: any,
  ...
  try {
    ...
    if (LdContact.message === "created" || LdContact.message === "exist") {
      ...
      const LdUpdateEventParticipant = await UpdateEventParticipant(
        idData.id,
        LdContact.data.name
      );
      return {
 ...
 */
export async function fnSubmitAppointmentBooking(
  idFormData: any,
  iRecaptchaToken: string,
  idConfig: TformConfig
) {
  try {
    ...
    const LTitle =
      LStatus === "Unverified"
        ? (idConfig.unVerifiedMessage?.label ?? "Welcome To Our Family!")
        : (idConfig.verifiedMessage?.label ?? "Thank You!");

    const LMessage =
      LStatus === "Unverified"
        ? (idConfig.unVerifiedMessage?.description ??
          "Please check your email to confirm the appointment")
        : (idConfig.verifiedMessage?.description ??
 ...
 * returns Object containing response data or error
 */
export async function fnSubmitContact(
  idFormData: any,
  iRecaptchaToken: string
) {
  try {
...
export async function fnDownload(
  ...
  try {
    ...
    const { PdfDocument } = await import(
      "@repo/ui/components/pdf/caseStudyLayout"
    );
...
function InnerSectionForm({
    ...
    const fnLoadTimeSlots = async (): Promise<void> => {
      if (SelectedDate && SelectedTimezone) {
        ...
        try {
          ...
          const LdSlotResult = await fetchTimeSlots(
            LFormattedDate,
            SelectedTimezone
          );
  ...
  const fnHandleSubmit = async (idFormData: z.infer<typeof config.schema>) => {
    ...
    try {
      ...
      if (config.formId === "booking") {
        LdResponse = await fnSubmitAppointmentBooking(
          idFormData,
          LdRecaptchaToken,
          config
        );
        ...
        return (
          <FormField
            ...
            render={({ field: iField, fieldState: iFieldState }) => (
              <FormItem
                className={fnGetClassNameFromFriendlyName(idField.fieldDisplay)}
              >
                <FormControl>
                  <FloatingLabelInput
            ...
            render={({ field: iField }) => (
              ...
              >
                <Select
                  ...
                  value={iField.value || idField.defaultValue}
                >
                  <FormControl>
              ...
              >
                <FormControl>
                  <Textarea
                    ...
                    className={cn(
                      "min-h-[100px]",
                      idField.inputClassName,
                      iFieldState.error && "border-red-400"
                    )}
                    {...iField}
                    value={iField.value || ""}
                  />
                    ...
                    <FormControl>
                      <Button
                        ...
                        className={cn(
                          ...
                          !iField.value && "text-muted-foreground"
                        )}
                      >
                        {iField.value
                          ? format(iField.value, "PPP")
                          : idField.placeholder}
                ...
                <div className="grid grid-cols-3 gap-2">
                  {IsLoadingSlots ? (
                    <p className="col-span-3 text-muted-foreground text-sm">
                      {idField.loading.label ?? "Loading slots..."}
                    </p>
                  ) : TimeSlots.length === 0 ? (
                    ...
                    TimeSlots.map(({ time, availability }) => {
                      //here the variable is destructure from response
                      const fromTime = time.slice(11, 16);
                      const [hourStr = "00", minuteStr = "00"] =
                        fromTime.split(":");
  ...
  return (
    <div
      ref={FormRef}
      className={cn(
        "w-full max-w-xl mx-auto shadow-md border border-border",
        className
      )}
    >
      {!hideCardHeader && (
        ...
        <Form {...LdForm}>
          <form onSubmit={LdForm.handleSubmit(fnHandleSubmit)}>
            ...
            <div className="flex flex-wrap -mx-2">
              {config.fields.map(fnRenderField)}
            </div>
            ...
            <div className="space-y-4">
              <Button
                ...
                disabled={
                  IsSubmitting ||
                  !LdForm.formState.isValid ||
                  Object.keys(LdForm.formState.dirtyFields).length === 0
                }
              >
                {IsSubmitting ? (
