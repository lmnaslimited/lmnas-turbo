...
import { TlocationCard, Tbutton, Titems } from "@repo/middleware/type";

export default function LocationCard({
  idLocation,
}: {
  idLocation: TlocationCard;
}) {
  ...
  return (
    <div className="relative">
      <div className="grid lg:grid-cols-12 items-start">
        <div
          className={`lg:col-span-5 ${idLocation.isReversed ? "lg:pl-5 order-1 lg:order-2" : "lg:pr-5"}`}
        >
          <div className="relative">
            ...
            <p className="text-foreground text-lg leading-relaxed mb-12">
              {idLocation.description}
            </p>

            <div className="space-y-6">
              {idLocation.contacts.some(
                (lContact) => lContact.type === "address"
              ) &&
                (() => {
                  const lAddress = idLocation.contacts.find(
                    (lContact) => lContact.type === "address"
                  );
                    ...
                    <div className="flex items-start gap-4 p-6 bg-accent border border-accent backdrop-blur-sm">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <button
                            onClick={() =>
                              fnCopyAddress(lAddress.description || "")
                            }
