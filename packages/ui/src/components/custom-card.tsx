...
export default function CustomCard({
  ...
  const fnGetAspectRatioClass = (): string => {
    if (
      !idCardProps.image?.aspectRatio ||
      idCardProps.image.aspectRatio === "auto"
    )
      return "";
    ...
    >
      {idCardProps.avatar &&
        (idCardProps.namePosition === "left" ||
          ...
          <Avatar>
            <AvatarImage
              src={idCardProps.avatar.source}
              alt={idCardProps.avatar.alternate}
            />
    ...
    >
      <div
        className={cn(
          ...
          idCardProps.layout === "horizontal" && "md:flex-1"
        )}
      >
            ...
            <>
              <h2
                className={cn(
                  ...
                  idCardProps.header.headingClass
                )}
              >
              ...
              <p
                className={cn(
                  "max-w-[700px] text-muted-foreground line-clamp-3",
                  idCardProps.header.descripClass
                )}
              >
        ...
        >
          <div
            className={cn(
              "flex w-full gap-8",
              idCardProps.buttons?.length === 1 ||
                idCardProps.link?.length === 1
                ? ""
            ...
            {idCardProps.buttons?.map((idBtn, iIndex) =>
              idBtn.href ? (
                ...
                <Button
                  ...
                  size={idBtn.size || "default"}
                  onClick={() =>
                    idCardProps.onButtonClick?.(idBtn.formMode, idBtn.label)
                  }
                >
                  {/* If iconPosition is 'before', render icon first */}
                  {idBtn.icon && idBtn.iconPosition === "before" && (
