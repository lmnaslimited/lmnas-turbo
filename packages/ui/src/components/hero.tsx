...
import type { TheroProps } from "../type.js";
...
export default function Hero({ iHero, onButtonClick }: HeroProps): ReactElement {
    ...
    <div className="container grid gap-12 lg:grid-cols-2 lg:gap-8 xl:gap-16 items-center md:py-24 lg:py-32 py-20">
      <div className="flex flex-col justify-center space-y-8">
        ...
        <div className="space-y-4">
          <h1
            className={`text-4xl font-bold tracking-tight text-primary sm:text-5xl md:text-6xl lg:text-7xl ${iHero.heading.headingClass}`}
          >
          ...
          {iHero.heading.subtitle && (
            <p
              className={`max-w-xl text-xl text-primary/70 md:text-2xl ${iHero.heading.descripClass}`}
            >
              {iHero.heading.subtitle}
            </p>
          )}
        ...
        <div className="flex flex-col gap-4 sm:flex-row">
          {iHero.buttons.map((idButton, index) =>
            idButton.href ? (
