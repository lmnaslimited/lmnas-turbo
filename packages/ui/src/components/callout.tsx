...
export default function Callout({idCallout}:{idCallout: TcalloutProps}) {
  if(!idCallout.layout){
     idCallout.layout == "classic"
  }
  return (
    <div className={`${idCallout.layout === "classic" ? "max-w-3xl" : "max-w-7xl"} mx-auto text-center py-16 px-4 sm:py-20 sm:px-6 lg:px-8`}>
        {idCallout.layout === "classic" ? (<h2 className={clsx("text-3xl font-extrabold sm:text-4xl", idCallout.variant || "text-secondary")}>
        ...
        <p className="mt-4 text-lg leading-6 text-secondary">{idCallout.points?.title}</p>
