...
import { TformFieldConfig } from "@repo/middleware/types"
    ...
    switch (name) {
      ...
      case "name":
        LdShape[name] = required
          ? z
              ...
              .regex(
                /^[A-Za-z\s]+$/,
                requiredError || "Name can only contain letters and spaces"
              )
          : z
              .string()
              .regex(
      ...
      case "email":
        LdShape[name] = required
          ...
          : z
              .string()
              .email(requiredError || "Please enter a valid email")
              .optional()
        break
